...
sampling = input("Sample data (rows) for faster runtime; enter proportion (e.g., 0.1) or leave blank to keep all:")
...
record_all = input("Record results for all networks generated for all columns (Y):")

#Define Zeta Type if not default
zeta_type = input("Zeta coefficient is supplied (S), manual input (M), default (D):")
zeta_list = []

if zeta_type == 'S' or zeta_type == 's':
    zeta_file = input("Name of zeta coefficients csv file:")
    zeta_name = pd.read_csv(zeta_file + '.csv', header=None)
    zeta_list = [float(zeta_name.iloc[x,0]) for x in arange(len(zeta_name.index))]
elif zeta_type == 'M' or zeta_type == 'm':
    zeta_list = [float(var) for var in raw_input("Enter coefficients (no comma between numbers):").split()]

zeta_thresh = input("Threshold number to determine optimal zeta coefficient (default if left blank):")


#Collect optional information for graph aesthetics
option = input("Enter optional information for graph scales and labels (Y):")

if option == 'Y' or option == 'y':
    x_label = input("Label for x axis:")
    coef_show = input("Show zeta coefficient on figure (Y):")
    column_name = input("Add column name as title (Y):")
    mode_name = input("Show mode name (Y):")
    mode_value = input("Show mode value (Y):")
    x_scale = input("Log (L) scale for x axis (leave blank for normal):")
    y_scale = input("Log (L) scale for y axis (leave blank for normal):")
    x_limits = input("Set boundaries for x axis (Y):")
    if x_limits == 'Y' or x_limits == 'y':
        x_limits1 = input("   Lower boundary:")
        x_limits2 = input("   Higher boundary:")
    else:
        x_limits1 = ""
        x_limits2 = ""
    y_limits = input("Set boundaries for y axis (Y):")
    if y_limits == 'Y' or y_limits == 'y':
        y_limits1 = input("   Lower boundary:")
        y_limits2 = input("   Higher boundary:")
    else:
        y_limits1 = ""
        y_limits2 = ""
    grid_lines = input("Show gridlines (Y):")
...
for column in arange(len(data.columns.values)):
    ...
    if zeta_type != 'S' and zeta_type != 's' and zeta_type != 'M' and zeta_type != 'm':
        from NFA_zeta import zeta_default
        zeta_list = zeta_default(pd.to_numeric(data.iloc[:,column]).dropna())
    ...
    for i in arange(len(zeta_list)):
        temp_results.append(network_calc(pd.to_numeric(data.iloc[:,column]).dropna(), zeta_list[i], column)) 
        if record_all == 'Y':
