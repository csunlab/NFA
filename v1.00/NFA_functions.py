...
version 1.00 - test to work with pandas version 0.14.1
'''
...
def save_prop(self, file_name, current):
    prop_file = open(file_name + '/' + file_name + '_coef_prop_' + str(current) + '.csv', 'wb')
    try:
...
def save_individual_results(self, coef, file_name, current):
    coef_data_file = open(file_name + '/' + file_name + '_coef_data_' + str(current) + '_' + str(coef) + '.csv', 'wb')
    try:
